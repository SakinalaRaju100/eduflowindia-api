...
exports.getAttendance = async (req, res) => {
  try {
    ...
    if (!attendance) {
      ...
      const records = (classroom?.students || []).map((s) => ({
        studentId: s,
        status: null,
      }));
      return res.json({
        success: true,
        data: { date, classroom: classroomId, records, isSubmitted: false },
      });
    }
...
exports.saveAttendance = async (req, res) => {
  try {
    ...
    const existing = await Attendance.findOne({
      ...
      date: {
        $gte: attendanceDate,
        $lte: new Date(attendanceDate.getTime() + 86400000),
      },
    });
...
exports.getStudentAttendance = async (req, res) => {
  try {
    ...
    const studentRecords = records.map((a) => ({
      date: a.date,
      status:
        a.records.find((r) => r.studentId.toString() === studentId)?.status ||
        null,
    }));
    ...
    res.json({
      success: true,
      data: { records: studentRecords, percentage, total, present },
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
...
exports.getInstitutionAttendanceSummary = async (req, res) => {
  try {
