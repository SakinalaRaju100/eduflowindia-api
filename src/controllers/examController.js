...
exports.createExam = async (req, res) => {
  try {
    ...
    if (!classroom) return res.status(400).json({ success: false, message: 'Classroom not found' });
...
exports.updateExam = async (req, res) => {
  try {
    const exam = await Exam.findOneAndUpdate({ _id: req.params.id, school: req.user.school }, req.body, { new: true, strict: false }).lean();
...
exports.enterResults = async (req, res) => {
  try {
    ...
    const ops = results.map(r => {
      const totalMarks = r.subjectMarks.reduce((s, m) => s + (m.marksObtained || 0), 0);
      const maxTotalMarks = r.subjectMarks.reduce((s, m) => s + (m.maxMarks || 100), 0);
      const percentage = maxTotalMarks > 0 ? Math.round((totalMarks / maxTotalMarks) * 100) : 0;
      const gpa = percentage >= 90 ? 10 : percentage >= 80 ? 9 : percentage >= 70 ? 8 : percentage >= 60 ? 7 : percentage >= 50 ? 6 : percentage >= 40 ? 5 : 4;
      ...
      return {
        updateOne: {
          ...
          update: {
            $set: {
              school: req.user.school, classroom: exam.classroom, academicYear: exam.academicYear || '',
              subjectMarks: r.subjectMarks, totalMarks, maxTotalMarks, percentage, gpa, overallGrade,
              isPassed: percentage >= 35, enteredBy: req.user._id,
            }
          },
          upsert: true,
        }
    ...
    // Calculate ranks
    const allResults = await Result.find({ exam: exam._id }).sort('-percentage');
    const rankOps = allResults.map((res, i) => ({
      updateOne: { filter: { _id: res._id }, update: { $set: { rank: i + 1 } } }
    }));
...
exports.getStudentResults = async (req, res) => {
  try {
    const results = await Result.find({ student: req.params.studentId, school: req.user.school })
