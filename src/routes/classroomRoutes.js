...
r1.post('/:id/students', authorize('principal', 'teacher'), async (req, res) => {
  try {
    const { studentIds, action } = req.body; // action: 'add' | 'remove'
    const op = action === 'remove' ? { $pull: { students: { $in: studentIds } } } : { $addToSet: { students: { $each: studentIds } } };
    const classroom = await Classroom.findByIdAndUpdate(req.params.id, op, { new: true });
    res.json({ success: true, data: classroom });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
});
